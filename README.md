...
The QWIKGAME map uses a number of different marks:
- Region-marks each represent multiple QWIKGAME Venues in a concentrated physical area. As the Map zooms-in, Region-markers are replaced by a combination of smaller Region-markers and/or Venue-markers  
- Venue-marks represents a single QWIKGAME Venue
- Place-marks represents a GOOGLE Place, each with a unique place-id. Google can provide a range of free/paid data for each place-id. A QWIKGAME Venue is created from this Google Place data.   