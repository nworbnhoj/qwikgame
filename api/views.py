...
from qwikgame.constants import ADMIN1, AVOIDABLE, COUNTRY, ERRORS, GAME, LAT, LNG, LOCALITY, MARKS, MSG, POS, REGION, STATUS
    ...
    def post(self, request, *args, **kwargs):
        ...
        if region:
            ...
            if country:
                ...
                if admin1:
                    ...
                    if locality:
                        region_objects = region_objects.filter(
                            locality=locality)
                    else:
                        region_objects = region_objects.filter(
        ...
        if avoidable:
            for avoid in avoidable.rsplit(':'):
                ...
                if locality:
                    mark_objects = mark_objects.exclude(
                        place__country=country, place__admin1=admin1, place__locality=locality)
                elif admin1:
                    mark_objects = mark_objects.exclude(
                        place__country=country, place__admin1=admin1)
                elif country:
        ...
        for region in regions:
            ...
            if locality:
                # get Venue Marks
                venue_marks = mark_objects.filter(
                    place__country=country, place__admin1=admin1, place__locality=locality)
                marks = marks | {vm.key(): vm.mark()
                                 for vm in venue_marks.all()}
            if not game:
                mark_objects = mark_objects.filter(game__isnull=True)
            if admin1:
                # get locality marks
                locality_marks = mark_objects.filter(
                    place__country=country, place__admin1=admin1, place__locality__isnull=False)
                marks = marks | {lm.key(): lm.mark()
                                 for lm in locality_marks.all()}
            if country:
                # get admin1 marks
                admin1_marks = mark_objects.filter(
                    place__country=country, place__admin1__isnull=False, place__locality__isnull=True)
                marks = marks | {am.key(): am.mark()
                                 for am in admin1_marks.all()}
            # get country marks
            country_marks = mark_objects.filter(
                place__country__isnull=False, place__admin1__isnull=True, place__locality__isnull=True)
