...
class Alert(models.Model):
    ...
    def str(self, on=True, keys='', type='_', route='all'):
        if on:
            list = [v.split('_')[0] for k, v in Alert.TYPE.items()
                    if k in keys and type in v]
        else:
            list = [v.split('_')[0] for k, v in Alert.TYPE.items()
                    if k not in keys and type in v]
        return ' '.join(list)
...
class Person(models.Model):
    block = models.ManyToManyField(
        'self', blank=True, symmetrical=False, through='Block')
    ...
    def alert(self,
        ...
        if type in self.notify_email:
            ...
            if not alert.dispatch():
                #        alert.save()
                logger.warn(
                    "Alert discarded - TODO serialize, save & replay unsent Alerts")
...
class Block(models.Model):
    person = models.ForeignKey(
        Person, on_delete=models.CASCADE, related_name='blocker')
    blocked = models.ForeignKey(
        Person, on_delete=models.CASCADE, related_name='blockee')
