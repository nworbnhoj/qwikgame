...
function winReady(callbackFunction) {
    if (document.readyState == 'complete') callbackFunction(event);
    else window.addEventListener("load", callbackFunction);
}
...
function nextDetail(event) {
    ...
    let i = detail_id.findIndex(function(detail) {
        return detail.id == current_id;
    });
...
function toggle_valid(toggle) {
    try {
        if (toggle.nodeType == "label" && toggle.classList.contains("toggle") && toggle.classList.firstElementChild.nodeType == "input") {
            alert("true");
            return true;
        }
    } finally {
...
docReady(event => {
    ...
    document.querySelectorAll('div.field div.label').forEach(
        (div_label) => {
            div_label.addEventListener('click', ({
                currentTarget
            }) => {
